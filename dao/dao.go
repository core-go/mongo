...
func NewMongoRepositoryWithVersion[T any, K any](db *mongo.Database, collectionName string, idObjectId bool, versionField string, options ...Mapper[T]) *Dao[T, K] {
	...
	if idIndex < 0 {
		log.Println(modelType.Name() + " Dao can't use functions that need Id value (Ex Load, Exist, Save, Update) because don't have any fields of " + modelType.Name() + " struct define _id bson tag.")
	}
	adapter := &Dao[T, K]{Collection: db.Collection(collectionName), idJson: jsonIdName, idIndex: idIndex, ObjectId: idObjectId,
...
func (a *Dao[T, K]) Create(ctx context.Context, model *T) (int64, error) {
	if a.Mapper != nil {
...
func (a *Dao[T, K]) Update(ctx context.Context, model *T) (int64, error) {
	if a.Mapper != nil {
		a.Mapper.ModelToDb(model)
	}
	vo := reflect.Indirect(reflect.ValueOf(model))
...
func (a *Dao[T, K]) Save(ctx context.Context, model *T) (int64, error) {
	if a.Mapper != nil {
		a.Mapper.ModelToDb(model)
	}
	vo := reflect.Indirect(reflect.ValueOf(model))
